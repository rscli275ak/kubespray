...
listen kubernetes-apiserver-https
    ...
    timeout client 3h
    timeout server 3h
    server wpkmaster wpkmaster:6443 check check-ssl verify none inter 10000
    balance roundrobin
...
for srv in $(cat /etc/hosts | grep wpkn | awk '{print $2}'); do echo "    server "$srv" "$srv":80 check" >>/etc/haproxy/haproxy.cfg
done
