...
IP_HAPROXY=$(dig +short wphaproxy)
IP_KMASTER=$(dig +short wpkmaster)
...
cat /etc/hosts | grep wpkm | awk '{print $2" ansible_host="$1" ip="$1" etcd_member_name=etcd"NR}'>kubespray/inventory/cluster/inventory.ini
cat /etc/hosts | grep wpkn | awk '{print $2" ansible_host="$1" ip="$1}'>>kubespray/inventory/cluster/inventory.ini

echo "[kube-master]">>kubespray/inventory/cluster/inventory.ini
cat /etc/hosts | grep wpkm | awk '{print $2}'>>kubespray/inventory/cluster/inventory.ini

echo "[etcd]">>kubespray/inventory/cluster/inventory.ini
cat /etc/hosts | grep wpkm | awk '{print $2}'>>kubespray/inventory/cluster/inventory.ini

echo "[kube-node]">>kubespray/inventory/cluster/inventory.ini
cat /etc/hosts | grep wpkn | awk '{print $2}'>>kubespray/inventory/cluster/inventory.ini
...
for srv in $(cat /etc/hosts | grep wpkm | awk '{print $2}'); do 
...
for srv in $(cat /etc/hosts | grep wpkn | awk '{print $2}'); do 
cat /home/vagrant/.ssh/id_rsa.pub | sshpass -p 'vagrant' ssh -o StrictHostKeyChecking=no vagrant@$srv -T 'tee -a >>/home/vagrant/.ssh/authorized_keys'
done
}
